...
require('../funciones.php'); // Assuming funciones.php contains obtenerEstante, obtenerOficina, obtenerPiso, obtenerEdificio, etc.

//verificarAdministrador();
$estante_id = $_GET['estante_id'];

// Obtener datos del estante, oficina, piso y edificio
$estante = obtenerEstante($estante_id);
$oficina = obtenerOficina($estante['oficina_id']);
$piso = obtenerPiso($oficina['piso_id']);
$edificio = obtenerEdificio($piso['edificio_id']);

// Obtener cajones del estante
...
<head>
    ...
    <title>Cajones de <?= htmlspecialchars($estante['codigo']) ?></title>
    ...
    <style>
        ...
        .submenu.open { display: block; animation: fadeInMenu 0.22s;}
...
<body>
    ...
    <div class="main-content" id="main-content">
        <div class="breadcrumbs">
            ...
            <span>&gt;</span>
            <a href="oficinas.php?piso_id=<?= $oficina['piso_id'] ?>"><?= htmlspecialchars($oficina['nombre']) ?></a>
            <span>&gt;</span>
            <a href="estantes.php?oficina_id=<?= $estante['oficina_id'] ?>">Estante <?= htmlspecialchars($estante['codigo']) ?></a>
            <span>&gt;</span>
            <span>Cajones</span>
        </div>
        ...
        <div class="header-section">
            <h1 class="mb-0">
                <i class="bi bi-grid-fill me-3"></i> Cajones del Estante <?= htmlspecialchars($estante['codigo']) ?>
            </h1>
            <a href="forms/cajon_form.php?estante_id=<?= $estante_id ?>" class="btn-add-new">
                <i class="bi bi-plus-circle me-2"></i>Nuevo Cajón
            </a>
        </div>
        
        <p class="text-muted mb-4">Ubicación: <strong><?= htmlspecialchars($edificio['nombre']) ?> &gt; Piso <?= $piso['numero'] ?> &gt; <?= htmlspecialchars($oficina['nombre']) ?> &gt; Estante <?= htmlspecialchars($estante['codigo']) ?></strong></p>
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        <?php while ($cajon = $result->fetch_assoc()): ?>
                        <tr>
                            <td><?= htmlspecialchars($cajon['codigo']) ?></td>
                            <td><?= htmlspecialchars($cajon['descripcion']) ?></td>
                            <td>
                                <a href="forms/cajon_form.php?id=<?= $cajon['id'] ?>" class="btn btn-sm btn-warning action-btn" title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <a href="acciones/eliminar_cajon.php?id=<?= $cajon['id'] ?>" 
                                   class="btn btn-sm btn-danger action-btn"
                                   onclick="return confirm('¿Eliminar este cajón?')"
                                   title="Eliminar">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="3" class="empty-state">
                                ...
                                <p>No hay cajones registrados en este estante.</p>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
