...
require('../funciones.php'); // Assuming funciones.php contains obtenerEdificio, obtenerPiso, etc.

//verificarAdministrador();
$edificio_id = $_GET['edificio_id'];

// Obtener datos del edificio
$edificio = obtenerEdificio($edificio_id);

// Obtener pisos del edificio
$query = "SELECT p.*, 
            (SELECT COUNT(*) FROM oficinas WHERE piso_id = p.id) AS total_oficinas
          FROM pisos p
          WHERE p.edificio_id = ?
          ORDER BY p.numero";
...
<head>
    ...
    <title>Pisos de <?= htmlspecialchars($edificio['nombre']) ?></title>
...
<body>
    ...
    <div class="main-content" id="main-content">
        <div class="breadcrumbs">
            ...
            <span>&gt;</span>
            <span>Pisos de <?= htmlspecialchars($edificio['nombre']) ?></span>
        </div>

        <div class="header-section">
            <h1 class="mb-0">
                <i class="bi bi-layers-fill me-3"></i> Pisos del Edificio: <?= htmlspecialchars($edificio['nombre']) ?>
            </h1>
            <a href="forms/piso_form.php?edificio_id=<?= $edificio_id ?>" class="btn-add-new">
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        <?php while ($piso = $result->fetch_assoc()): ?>
                        <tr>
                            <td><?= htmlspecialchars($piso['numero']) ?></td>
                            <td><?= htmlspecialchars($piso['descripcion']) ?></td>
                            <td><?= $piso['total_oficinas'] ?></td>
                            <td>
                                <a href="oficinas.php?piso_id=<?= $piso['id'] ?>" class="btn btn-sm btn-info action-btn" title="Ver Oficinas">
                                    <i class="bi bi-grid-3x3-gap-fill me-1"></i> Oficinas
                                </a>
                                <a href="forms/piso_form.php?id=<?= $piso['id'] ?>" class="btn btn-sm btn-warning action-btn" title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <a href="acciones/eliminar_piso.php?id=<?= $piso['id'] ?>" 
                                   class="btn btn-sm btn-danger action-btn"
                                   onclick="return confirm('¿Eliminar este piso y todas sus oficinas?')"
                                   title="Eliminar">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="4" class="empty-state">
                                ...
                                <p>No hay pisos registrados en este edificio.</p>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
