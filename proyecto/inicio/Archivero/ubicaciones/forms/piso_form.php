...
require('../../funciones.php'); // Asegúrate de que este archivo exista y contenga obtenerEdificio, obtenerPiso y mostrarMensajes

//verificarAdministrador(); // Uncomment this line if you need admin verification

$edificio_id = $_GET['edificio_id'] ?? 0;
$piso = ['id' => 0, 'numero' => '', 'descripcion' => '', 'edificio_id' => $edificio_id];
$titulo = "Nuevo Piso";
...
if (isset($_GET['id'])) {
    ...
    if (!$piso) {
        // Redirigir con un mensaje de error si el piso no se encuentra
        $_SESSION['mensaje'] = [
            'titulo' => 'Error',
            'contenido' => 'Piso no encontrado.',
            'tipo' => 'danger'
        ];
...
// Obtener datos del edificio padre
...
<body>
    <div class="container">
        <div class="breadcrumbs">
            ...
            <span>&gt;</span>
            <a href="../pisos.php?edificio_id=<?= htmlspecialchars($edificio['id']) ?>">Pisos de <?= htmlspecialchars($edificio['nombre']) ?></a>
            <span>&gt;</span>
            <span><?= $titulo ?></span>
        </div>
        ...
        <div class="form-container-custom">
            <div class="form-header">
                <h1>
                    <i class="bi bi-box-seam me-2"></i><?= $titulo ?>
                </h1>
                <a href="../pisos.php?edificio_id=<?= htmlspecialchars($edificio['id']) ?>" class="btn btn-secondary">
                    <i class="bi bi-arrow-left me-2"></i>Volver a Pisos
                </a>
            </div>
            ...
            <form action="../acciones/guardar_piso.php" method="POST">
                <input type="hidden" name="edificio_id" value="<?= htmlspecialchars($edificio['id']) ?>">
                
                <?php if ($piso['id'] > 0): ?>
                    <input type="hidden" name="id" value="<?= htmlspecialchars($piso['id']) ?>">
                <?php endif; ?>

                <div class="form-group">
                    <label for="numero">Número de Piso:</label>
                    <input type="text" id="numero" name="numero" class="form-control" value="<?= htmlspecialchars($piso['numero']) ?>" required>
                </div>

                <div class="form-group">
                    <label for="descripcion">Descripción:</label>
                    <textarea id="descripcion" name="descripcion" class="form-control" rows="3"><?= htmlspecialchars($piso['descripcion']) ?></textarea>
                </div>
