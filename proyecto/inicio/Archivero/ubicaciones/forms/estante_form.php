...
require('../../funciones.php'); // Asegúrate de que este archivo exista y contenga obtenerEdificio, obtenerPiso, obtenerOficina, obtenerEstante y mostrarMensajes

//verificarAdministrador(); // Uncomment this line if you need admin verification

$oficina_id = $_GET['oficina_id'] ?? 0;
$estante = ['id' => 0, 'codigo' => '', 'descripcion' => '', 'oficina_id' => $oficina_id];
$titulo = "Nuevo Estante";
...
if (isset($_GET['id'])) {
    ...
    if (!$estante) {
        // Redirigir con un mensaje de error si el estante no se encuentra
        $_SESSION['mensaje'] = [
            'titulo' => 'Error',
            'contenido' => 'Estante no encontrado.',
            'tipo' => 'danger'
        ];
    ...
    <div class="container">
        ...
        <div class="form-container-custom">
            <div class="form-header">
                <h1>
                    <i class="bi bi-card-checklist me-2"></i><?= $titulo ?>
                </h1>
                <a href="../estantes.php?oficina_id=<?= htmlspecialchars($oficina['id']) ?>" class="btn btn-secondary">
                    <i class="bi bi-arrow-left me-2"></i>Volver a Estantes
                </a>
            </div>
            ...
            <p class="text-muted mb-4">Ubicación: <strong><?= htmlspecialchars($edificio['nombre']) ?> &rarr; Piso <?= htmlspecialchars($piso['numero']) ?> &rarr; <?= htmlspecialchars($oficina['nombre']) ?></strong></p>

            <form action="../acciones/guardar_estante.php" method="POST">
                <input type="hidden" name="oficina_id" value="<?= htmlspecialchars($oficina['id']) ?>">
                
                <?php if ($estante['id'] > 0): ?>
                    <input type="hidden" name="id" value="<?= htmlspecialchars($estante['id']) ?>">
                <?php endif; ?>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" id="codigo" name="codigo" class="form-control"
                               value="<?= htmlspecialchars(str_replace('EST-', '', $estante['codigo'])) ?>"
                               required pattern="[A-Z0-9-]+" title="Solo letras mayúsculas, números y guiones">
                    </div>
                    <small class="form-text">El sistema agregará automáticamente el prefijo "EST-"</small>
                </div>

                <div class="form-group">
                    <label for="descripcion">Descripción:</label>
                    <textarea id="descripcion" name="descripcion" class="form-control" rows="3"><?= htmlspecialchars($estante['descripcion']) ?></textarea>
                </div>
