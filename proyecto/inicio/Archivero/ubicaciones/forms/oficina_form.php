...
require('../../funciones.php'); // Asegúrate de que este archivo exista y contenga obtenerEdificio, obtenerPiso, obtenerOficina y mostrarMensajes

//verificarAdministrador(); // Uncomment this line if you need admin verification

$piso_id = $_GET['piso_id'] ?? 0;
$oficina = ['id' => 0, 'codigo' => '', 'nombre' => '', 'piso_id' => $piso_id];
...
if (isset($_GET['id'])) {
    ...
    if (!$oficina) {
        // Redirigir con un mensaje de error si la oficina no se encuentra
        $_SESSION['mensaje'] = [
            'titulo' => 'Error',
            'contenido' => 'Oficina no encontrada.',
            'tipo' => 'danger'
        ];
    ...
    <div class="container">
        ...
        <div class="form-container-custom">
            <div class="form-header">
                <h1>
                    <i class="bi bi-grid-3x3-gap-fill me-2"></i><?= $titulo ?>
                </h1>
                <a href="../oficinas.php?piso_id=<?= htmlspecialchars($piso['id']) ?>" class="btn btn-secondary">
                    <i class="bi bi-arrow-left me-2"></i>Volver a Oficinas
                </a>
            </div>
            ...
            <p class="text-muted mb-4">Ubicación: <strong><?= htmlspecialchars($edificio['nombre']) ?> &rarr; Piso <?= htmlspecialchars($piso['numero']) ?></strong></p>

            <form action="../acciones/guardar_oficina.php" method="POST">
                <input type="hidden" name="piso_id" value="<?= htmlspecialchars($piso['id']) ?>">
                
                <?php if ($oficina['id'] > 0): ?>
                    <input type="hidden" name="id" value="<?= htmlspecialchars($oficina['id']) ?>">
                <?php endif; ?>

                <div class="form-group">
                    <label for="codigo">Código:</label>
                    <input type="text" id="codigo" name="codigo" class="form-control" value="<?= htmlspecialchars($oficina['codigo']) ?>" required>
                </div>

                <div class="form-group">
                    <label for="nombre">Nombre:</label>
                    <input type="text" id="nombre" name="nombre" class="form-control" value="<?= htmlspecialchars($oficina['nombre']) ?>" required>
                </div>
