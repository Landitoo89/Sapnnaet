...
require('../funciones.php'); // Assuming funciones.php contains obtenerEdificio, obtenerPiso, obtenerOficina, etc.

//verificarAdministrador();
$piso_id = $_GET['piso_id'];

// Obtener datos del piso y edificio
$piso = obtenerPiso($piso_id);
$edificio = obtenerEdificio($piso['edificio_id']);

// Obtener oficinas del piso
$query = "SELECT o.*, 
            (SELECT COUNT(*) FROM estantes WHERE oficina_id = o.id) AS total_estantes
          FROM oficinas o
          WHERE o.piso_id = ?
          ORDER BY o.codigo";
$stmt = $conexion->prepare($query);
$stmt->bind_param('i', $piso_id);
$stmt->execute();
...
<head>
    ...
    <title>Oficinas del Piso <?= $piso['numero'] ?> - <?= htmlspecialchars($edificio['nombre']) ?></title>
...
<body>
    ...
    <div class="main-content" id="main-content">
        ...
        <div class="header-section">
            <h1 class="mb-0">
                <i class="bi bi-grid-3x3-gap-fill me-3"></i> Oficinas del Piso <?= $piso['numero'] ?>
            </h1>
            <a href="forms/oficina_form.php?piso_id=<?= $piso_id ?>" class="btn-add-new">
                <i class="bi bi-plus-circle me-2"></i>Nueva Oficina
            </a>
        </div>
        
        <p class="text-muted mb-4">Edificio: <strong><?= htmlspecialchars($edificio['nombre']) ?></strong></p>
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        <?php while ($oficina = $result->fetch_assoc()): ?>
                        <tr>
                            <td><?= htmlspecialchars($oficina['codigo']) ?></td>
                            <td><?= htmlspecialchars($oficina['nombre']) ?></td>
                            <td><?= $oficina['total_estantes'] ?></td>
                            <td>
                                <a href="estantes.php?oficina_id=<?= $oficina['id'] ?>" class="btn btn-sm btn-info action-btn" title="Ver Estantes">
                                    <i class="bi bi-card-checklist me-1"></i> Estantes
                                </a>
                                <a href="forms/oficina_form.php?id=<?= $oficina['id'] ?>" class="btn btn-sm btn-warning action-btn" title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <a href="acciones/eliminar_oficina.php?id=<?= $oficina['id'] ?>" 
                                   class="btn btn-sm btn-danger action-btn"
                                   onclick="return confirm('¿Eliminar esta oficina y todos sus estantes?')"
                                   title="Eliminar">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="4" class="empty-state">
                            ...
                            <p>No hay oficinas registradas en este piso.</p>
                        </td>
                    </tr>
                <?php endif; ?>
                </tbody>
